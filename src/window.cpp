...
void Window::init()
{
	...
	songs_start = false;
	try {
		if(!songs.openFromFile(song_paths[stoi(opt[3])])) 
			if(!songs.openFromFile(song_paths[0]))
				songs_start = true;
	} catch(...) {
		songs_start = true;
	}
...
void Window::saveScore() {
	int value_to_change;
	std::string values[8];
	std::ifstream file1("../resources/score.bin", std::ios::binary);
	int i = 0;
	while(file1) {
		getline(file1, values[i]);
		i++;
		if(i == 8) break;
	}
	if(i < 8) for(int k = 0; k < 8; k++) values[k] = "0";
	file1.close();
	std::ofstream file2("../resources/score.bin", std::ios::binary);
	switch(static_cast<int>(game.table_dimensions)) {
		case 8:
			if(game.isWallCollidable) value_to_change = 4;
			else value_to_change = 0;
			break;
		case 10:
			if(game.isWallCollidable) value_to_change = 5;
			else value_to_change = 1;
			break;
		case 12:
			if(game.isWallCollidable) value_to_change = 6;
			else value_to_change = 2;
			break;
		case 14:
			if(game.isWallCollidable) value_to_change = 7;
			else value_to_change = 8;
			break;
	}
	if(stoi(values[value_to_change]) < game.points) {
		values[value_to_change] = std::to_string(game.points);
	}
	for(int j = 0; j < 8; j++) {
		file2 << values[j] << std::endl;	
	}
    file2.close();
}
...
void Window::recordsLoop()
{
	game_records.input();
	game_records.draw(mWindow);
	if(game_records.response() == 1) {init(); game_state = Menu;}
}
